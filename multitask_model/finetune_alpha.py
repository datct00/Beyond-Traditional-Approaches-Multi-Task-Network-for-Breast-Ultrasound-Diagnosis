...
from utils import AverageMeter, setup_logger, logging_hyperparameters
...
def train():
    ...
    #Save init weight model 
    INIT_MODEL_PATH = "tmp/init.pth"
    ...
    for alpha in ALPHA:
        ...
        stale = 0
        #Setup logging
        WEIGHT_DIR = os.path.join(OUTPUT_DIR, f"{ENCODER_NAME}_{ARCH}",f"alpha={alpha}")
        if not os.path.exists(WEIGHT_DIR):
            os.makedirs(WEIGHT_DIR, exist_ok=True)
        LOG_DIR = WEIGHT_DIR
        LOGGER_NAME = f"{ENCODER_NAME}_{ARCH}_alpha={alpha}"
        ...
        #Load init weight model for new alpha
        ...
        for epoch in range(start_epoch, 1+MAX_EPOCHS):
            ...
            if overall_meter.avg > best_overall: # best base on IoU score
                ...
                torch.save(to_save, os.path.join(WEIGHT_DIR,f"best_{epoch}_{INPUT_SIZE[0]}_BS={BATCH_SIZE}_overall={overall_meter.avg:.3f}_alpha={alpha}.pth"))
